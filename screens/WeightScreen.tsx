...
export const WeightScreen: React.FC<WeightScreenProps> = ({ navigation, route }) => {
  ...
  const [weight, setWeight] = useState<number | null>(150); // Default: 150 lbs

  const lbsItems = Array.from({ length: 300 }, (_, i) => i + 50);
  const kgItems = Array.from({ length: 200 }, (_, i) => i + 20);

  const weightItems = unit === 'lbs' ? lbsItems : kgItems;

  const isValid = weight !== null;
      ...
      <View style={styles.content}>
        ...
        <View style={styles.unitToggle}>
          <TouchableOpacity
            ...
            onPress={() => {
              setUnit('lbs');
              // Set default if not already set or if current value is out of range
              if (weight === null || (weight < 50 || weight > 349)) {
                setWeight(150);
              }
            }}
          >
          ...
          <TouchableOpacity
            ...
            onPress={() => {
              setUnit('kg');
              // Convert lbs to kg if switching, or set default
              if (weight !== null && unit === 'lbs') {
                const kgValue = Math.round(weight * 0.453592);
                setWeight(Math.max(20, Math.min(219, kgValue)));
              } else if (weight === null || (weight < 20 || weight > 219)) {
                setWeight(68); // ~150 lbs in kg
              }
            }}
          >
        ...
        <View style={styles.pickerContainerWrapper}>
          <ScrollablePicker
            ...
            label={unit === 'lbs' ? 'pounds' : 'kilograms'}
          />
        </View>
      </View>
